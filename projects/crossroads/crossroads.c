...
void check_parse_vehicles(struct vehicle_info *vehicle_info, int thread_cnt)
{
    for (int i = 0; i < thread_cnt; i++)
    {
        printf(
            "id = %c, start = %c, dest = %c, type = %d, arrival = %d, golden_time = %d, state = %d\n",
            vehicle_info[i].id,               // 차량 ID
            vehicle_info[i].start,            // 출발 노드
            vehicle_info[i].dest,             // 도착 노드
            (int)vehicle_info[i].type,        // VEHICL_TYPE_NORMAL(0) 또는 VEHICL_TYPE_AMBULANCE(1)
            (int)vehicle_info[i].arrival,     // 정수형 도착 시간(없으면 0)
            (int)vehicle_info[i].golden_time, // 정수형 골든 타임(없으면 0)
            (int)vehicle_info[i].state        // VEHICLE_STATUS_READY(0), RUNNING(1), FINISHED(2)
        );
    }
}
